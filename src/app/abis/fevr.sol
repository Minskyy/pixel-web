...
// File: bepro-js/contracts/ERC721Marketplace.sol
...
contract ERC721Marketplace is Ownable {
    ...
    constructor(ERC20 _erc20Address, IERC721 _erc721Address) public {
    ...
    function putERC721OnSale(uint256 _tokenId, uint256 _price) public {
        require(
            erc721Address.ownerOf(_tokenId) == msg.sender,
    ...
    function buyERC721(uint256 _tokenId) public payable virtual {
        require(sales[_tokenId].tokenId == _tokenId, "NFT is not in sale");
        ...
        if (isNativeTransaction()) {
            ...
            require(
                sales[_tokenId].price == msg.value,
            ...
            require(
                sales[_tokenId].seller.send(
                    ((100 - feePercentage) * sales[_tokenId].price) / 100
                ),
                "Wasnt able to transfer the Native Currency to the seller"
            );
        } else {
    ...
    function setFixedFees(address payable _feeAddress, uint256 _feePercentage)
        public
        onlyOwner
    {
        ...
        require(
            _feePercentage < 100,
            "Fee Percentage has to be lower than 100"
        );
...
// File: bepro-js/contracts/custom/MarketplaceRealFvr.sol

pragma solidity >=0.6.0;

interface OpenerRealFvr is IERC721 {
    function getMarketplaceDistributionForERC721(uint256 tokenId)
        external
        returns (uint256[] memory, address[] memory);
}

contract MarketplaceRealFevrV2 is ERC721Marketplace {
    OpenerRealFvr public erc721Address;

    constructor(ERC20 _erc20Address, OpenerRealFvr _erc721Address)
        public
    ...
    function buyERC721(uint256 _tokenId) public payable virtual override {
        require(sales[_tokenId].tokenId == _tokenId, "NFT is not in sale");
        require(!sales[_tokenId].sold, "NFT has to be available for purchase");
        ...
        (distributionAmounts, distributionAddresses) = erc721Address
            .getMarketplaceDistributionForERC721(_tokenId);
        ...
        if (isNativeTransaction()) {
            ...
            for (uint256 i = 0; i < distributionAmounts.length; i++) {
                if (distributionAddresses[i] != address(0)) {
                    // Transfer fee to fee address
                    require(
                        payable(distributionAddresses[i]).send(
                            (distributionAmounts[i] * sales[_tokenId].price) /
                                100
                        ),
                        "Contract was not allowed to do the transfer"
                    );
            ...
            require(
                sales[_tokenId].seller.send(
                    ((100 - feePercentage - totalFee) * sales[_tokenId].price) /
                        100
                ),
                "Wasnt able to transfer the Native Currency to the seller"
            );
