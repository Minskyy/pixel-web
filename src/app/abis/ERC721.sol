    ...
    IERC721Enumerable
{
     ...
     * Emits a {Transfer} event.
     */
    function _burn2(uint256 tokenId) internal virtual {
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        require(
            ...
            "ERC721: Trying to burn not owned token"
        );
...
// File: contracts/custom/Pixel.sol

pragma solidity >=0.6.0;

contract Pixel is Ownable, ERC721 {
    using SafeMath for uint256;
    ...
    uint256 public constant areaPrice = 40 * (10**18);

    uint256 public packIncrementId = 1;
    uint256 public lastNFTID = 1;
    address private _pixelAddress;
    ...
    function burn(uint256 tokenIdToMint) public {
        require(
            ERC721.ownerOf(tokenIdToMint) == msg.sender,
            "ERC721: Trying to burn not owned token"
        );
        _burn2(tokenIdToMint);

        IERC20(_pixelAddress).transfer(msg.sender, areaPrice);

        emit NftBurned(tokenIdToMint);
    }
    ...
    function setTokenPriceInUSD(
        uint256 newPrice /* 1*10e18 -> 1 FVR = $1; 1*10e17 -> 0.1 FVR = $1  */
    ) public onlyOwner {
        require(newPrice != 0, "newPrice has to higher than 0");
        _realFvrTokenPriceUSD = newPrice;
    }

    function buyNftArea(uint256 areaIndex) public {
        IERC20(_pixelAddress).transferFrom(
            msg.sender,
            address(this),
            areaPrice
        );
    ...
    function changePixelAddress(address newPixelAddress) public onlyOwner {
        _pixelAddress = newPixelAddress;
    }
